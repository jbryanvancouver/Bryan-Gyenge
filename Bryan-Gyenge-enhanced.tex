...
\begin{proof}
	...
	Here $\langle \cdot, \cdot \rangle$ is the inner product with respect to the affine invariant form.
	...
	Theorem \ref{thm: Zorbifold formula} also implies that any such component has a positive Euler characteristic. This is because both the numerator and the expansion of the denominator have positive coefficients. The statement about the colengths follows from the equation $\mathbf{v}=\mathbf{m}+\delta \left( \frac{( \mathbf{m}| \mathbf{m}) }{2}\right)$ and the decomposition \[
	D_{\mdata}^{2} = -\sum_{i=1}^{r} \left(\mvec (i)|\mvec (i)
	\right)_{\Delta (i)}.
	\]
