...
X/G$ be the minimal resolution. Our approach to proving
...
We abreviate the data $\left\{m_{j}(i) \right\}$ by the symbol
$\mdata$ and we denote by 
\[
\Hilb^{n,\mdata}(\Zcal ) \subset \Hilb (\Zcal )
\]
the component corresponding to the $K$-theory class given above. We
may also regard $\Hilb^{n,\mdata}(\Zcal )$ as parameterizing ideal
...
\begin{lemma}
\begin{enumerate}
	\item Let $M$ be an ADE root lattice. To any element $\mathbf{m} \in M$ there is a rigid G invariant subscheme of $\mathbb{C}^2$ with $K$-theory class corresponding to $\mathbf{m}$.
	\item For every datum $\mdata$ there is a rigid subscheme of $\mathcal{Z}$ with $K$-theory class $0,\mdata$.
\end{enumerate}
\end{lemma}
\begin{proof}
...
Nakajima has shown in \cite[Section 2]{nakajima2002geometric} that the $G$-fixed Hilbert scheme decomposes into quiver varieties
...
Theorem \ref{thm: Zorbifold formula} also implies that any such component has a positive Euler characteristic. This is because both the numerator and the expansion of the denominator have positive coefficients.
...
With the proposition, we can now prove Theorem~\ref{thm: Formula for
...
the negative of the corresponding Cartan matrix $C_{\Delta (i)}$. Let
\[
