...
\subsection{Acknowledgements.} The authors warmly thank Federico
Amadio Guidi, Georg Oberdieck, Ken Ono, and Bal\'azs Szendr\H{o}i for
helpful comments.
...
\begin{lemma}
\begin{enumerate}
	\item To any element $\mathbf{m} \in M$ in the root lattice  there is a rigid G invariant subscheme of $\mathbb{C}^2$ associated with $\mathbf{m}$.
	\item For every datum $n, \mdata$ there is a rigid ideal sheaf
\end{enumerate}
\end{lemma}
\begin{proof}
It is due to Nakajima that 
$\mathrm{Hilb}(\mathbb{C}^2)^G = \bigsqcup_{\mathbf{v},\mathbf{w}} \mathcal{M}(\mathbf{v},\mathbf{w})$ where $\mathbf{w}=(1,0,\dots,0)$.
Due to Theorem \ref{thm: Zorbifold formula} a sheaf associated with $\mathbf{m}$ is in the component of $\mathrm{Hilb}(\mathbb{C}^2)^G$ whose dimension vector is
...
By 
\cite[Proposition 3.4]{Losev-Nakajima-quiver-varieties}
the dimension of a component with dimension vector $\mathbf{v}$ and framing vector $\mathbf{w}$ is
...
0  \end{align*}
where we have used that $\langle M, \delta \rangle = 0$ and $\langle \delta, \delta \rangle = 0$.
