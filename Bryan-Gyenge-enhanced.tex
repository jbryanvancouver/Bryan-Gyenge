...
the motivic class. These refinements all stem from the following
theorem which we prove in Section~\ref{sec: proof of thm about Zbir}.
...
product. Let $Y$ be the minimal resolution of $X/G$, then
\[
...
The above series has further specializations giving formulas for the  Hodge polynomials and
...
X/G$ be the minimal resolution. Let $p_{1},\dotsc ,p_{r}\in \Zcal$
be the orbifold points, and let $U\subset \Zcal \setminus \{p_{1},\dotsc ,p_{r} \}$ be
the  Zariski open part with trivial stabilizers. %This stack is equivalent to the scheme $W$.
Let furthermore $V \subset Y$ be the complement of the exceptional divisors.
Then $U$ and $V$ are canonically isomorphic.
...
is the trivial representation. We note that we have the $K$-theory relation
\[
[\O_{p}] = [\O_{p_{i}}\otimes \rho_{\reg}(i)]
\]
where $\rho_{\reg}(i)$ is the regular representation of $G_{\Delta
	(i)}$, which explains why we do not need to include $ [\O_{p_{i}}\otimes
\rho_{0}(i)]$ in our basis for $K$-theory.

We abbreviate the data $\left\{m_{j}(i) \right\}$ by the symbol
$\mdata$ and we denote by 
\[
\Hilb^{n,\mdata}(\Zcal ) \subset \Hilb (\Zcal )
\]
the component corresponding to the $K$-theory class given above.
...
\label{lem:rigidsh}
	\begin{enumerate}
		\item Let $M$ be an ADE root lattice. To any element $\mathbf{m} \in M$ there is a rigid G invariant subscheme of $\mathbb{C}^2$ with $K$-theory class corresponding to $\mathbf{m}$. The (graded) colength of the ideal of this subscheme is $\mathbf{m}+\half \delta( \mathbf{m}| \mathbf{m})$.
		\item For every datum $\mdata$ there is a rigid subscheme $Z_{\mdata} \subset \mathcal{Z}$ with $K$-theory class $\mdata$.
This subscheme has $\mathrm{colength}(\mathcal{I}_{Z_{\mdata}} \subset \mathcal{O}_{\Zcal})=\mdata -\half |G| D_{\mdata}^{2}$.
	\end{enumerate}
\end{lemma}
\begin{proof}
	Part (2) is implied by Part (1) since we can take the union of the rigid subschemes supported at the orbifold points $p_1,\dots,p_r \in \mathcal{Z}$. So we only need to prove the local case.
	...
	$\mathrm{Hilb}(\mathbb{C}^2)^G = \bigsqcup_{\mathbf{v}} \mathcal{M}(\mathbf{v},\mathbf{w})$ with fixed framing vector $\mathbf{w}=(1,0,\dots,0)$.
	Due to Theorem \ref{thm: Zorbifold formula} a sheaf with $K$-theory class described by the root $\mathbf{m} \in M$ is in the component of $\mathrm{Hilb}(\mathbb{C}^2)^G$ whose dimension vector is
	\[ \mathbf{v}=\mathbf{m}+\delta \left( \frac{\langle \mathbf{m}, \mathbf{m} \rangle }{2} \right)= \left(\frac{a_0\langle \mathbf{m}, \mathbf{m} \rangle }{2},m_1+\frac{a_1\langle \mathbf{m}, \mathbf{m} \rangle }{2},\dots,m_n+\frac{a_n\langle \mathbf{m}, \mathbf{m} \rangle }{2} \right). \]
	By 
	\cite[(2.6)]{nakajima1994instantons}
	the dimension of $\mathcal{M}(\mathbf{v},\mathbf{w})$ in general is
	\[ 2\mathbf{w} \cdot \mathbf{v} - \langle \mathbf{v}, \mathbf{v} \rangle.  \]
	In our case this reads as
	\begin{align*} 2v_0 - \langle \mathbf{v}, \mathbf{v} \rangle & = \langle \mathbf{m}, \mathbf{m} \rangle - \left\langle \mathbf{m}+\delta \left( \frac{\langle \mathbf{m}, \mathbf{m} \rangle }{2} \right), \mathbf{m}+\delta \left( \frac{\langle \mathbf{m}, \mathbf{m} \rangle }{2} \right) \right\rangle \\
	& =\langle \mathbf{m}, \mathbf{m} \rangle - \langle \mathbf{m}, \mathbf{m} \rangle -  \langle \mathbf{m}, \delta \rangle \langle \mathbf{m}, \mathbf{m} \rangle -\frac{1}{2} \langle \mathbf{m}, \mathbf{m} \rangle\langle \delta, \delta \rangle =
	0  \end{align*}
	where we have used that $\langle M, \delta \rangle = 0$ and $\langle \delta, \delta \rangle = 0$. Hence, any sheaf in such a component is rigid.
	
	Theorem \ref{thm: Zorbifold formula} also implies that any such component has a positive Euler characteristic. This is because both the numerator and the expansion of the denominator have positive coefficients. Part (3) \[
	D_{\mdata}^{2} = -\sum_{i=1}^{r} \left(\mvec (i)|\mvec (i)
	\right)_{\Delta (i)} 
	\]
...
Let $Z_{\mdata} \subset \mathcal{Z}$ be the rigid substack of $Z$ corresponding to the $K$-theory data $0,\mdata$ provided by Lemma \ref{lem:rigidsh}. 
...
D_{\mdata}^{2}}(U)$. This is because  $Z_{\mdata}$ was rigid and the colength of $\mathcal{I}_{Z_{\mdata}}$ is $\mdata -\half|G|
D_{\mdata}^{2}$. On the other hand, the Zariski open subset of $\mathrm{Hilb}^{n+\half
